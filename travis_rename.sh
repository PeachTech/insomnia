...
find ./packages/insomnia-app/dist -type f -name *.AppImage -execdir cp {} Peach-Sidecar-Setup-$ver-x86_64-linux.AppImage \;
find ./packages/insomnia-app/dist -type f -name *.snap -execdir cp {} Peach-Sidecar-Setup-$ver-x86_64-linux.snap \;
find ./packages/insomnia-app/dist -type f -name *.dmg -execdir cp {} Peach-Sidecar-Setup-$ver-x86_64-mac.dmg \;
